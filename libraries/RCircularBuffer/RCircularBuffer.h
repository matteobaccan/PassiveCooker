/*
 RCircularBuffer.h - Circular buffer library for Arduino.
 ...
 */
#ifndef RCIRCULAR_BUFFER_H_
#define RCIRCULAR_BUFFER_H_
#include <stdint.h>
#include <stddef.h>
...
template<typename T, size_t S, typename IT = typename Helper::Index<(S <= UINT8_MAX), (S <= UINT16_MAX)>::Type> class RCircularBuffer {
public:
	/**
	...
	constexpr RCircularBuffer();

	/**
	 * Disables copy constructor
	 */
	RCircularBuffer(const RCircularBuffer&) = delete;
	RCircularBuffer(RCircularBuffer&&) = delete;

	/**
	 * Disables assignment operator
	 */
	RCircularBuffer& operator=(const RCircularBuffer&) = delete;
	RCircularBuffer& operator=(RCircularBuffer&&) = delete;
...
private:
	...
	T *head;
	T *tail;
#ifndef RCIRCULAR_BUFFER_INT_SAFE
	IT count;
#else
	volatile IT count;
#endif
